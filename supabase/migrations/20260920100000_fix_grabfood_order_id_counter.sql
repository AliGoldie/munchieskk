-- log_grabfood_daily_entry() (20260919151217) minted its order id from
-- public.order_id_counters, a table 20260829000001 already dropped in favour of
-- per-day sequences, so every GrabFood entry failed with
-- 'relation "public.order_id_counters" does not exist'.
--
-- Same fix place_order() got: mint from a non-transactional per-day sequence,
-- seeded from any existing GRB-DDMM-### rows for that day.
