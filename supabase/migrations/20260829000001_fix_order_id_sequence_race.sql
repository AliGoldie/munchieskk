-- FOLLOW-UP FIX to 20260829000000_atomic_order_id_generation.sql, which
-- shipped a real bug of its own: it incremented order_id_counters inside
-- the SAME transaction as the orders INSERT. Postgres rolls back a failed
-- transaction in full, so whenever the id collided with a pre-existing row
-- (there was already a real MP-2908-001 order from before that fix went
-- live) the counter's own increment got rolled back too — every retry
-- recomputed the exact same doomed id forever. Confirmed live: calling
-- place_order() repeatedly always returned the identical
-- "duplicate key value violates unique constraint orders_pkey" for
-- MP-2908-001, never advancing.
--
-- FIX: use a native Postgres SEQUENCE per day instead of a table row.
-- Sequence advances (nextval()) are explicitly NON-transactional in
-- Postgres — they are never rolled back, even if the transaction that
-- called nextval() aborts. So a failed order can never cause two attempts
-- to compute the same counter value again. When the day's sequence is
-- first created, it is seeded past any orders already existing under
-- today's MP-<day>-NNN pattern (handles the pre-existing MP-2908-001 row,
-- or any other manual/legacy id) so the very first id generated after this
-- migration can't collide with it either.

DROP TABLE IF EXISTS public.order_id_counters;
